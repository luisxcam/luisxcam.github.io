  ...
  return (
    <TerminalWindow title="~/portfolio/playlist.json">
      ...
      <div className="border border-border">
        <div className="grid grid-cols-4 gap-0 border-b border-border bg-muted px-3 py-1.5 text-[10px] text-muted-foreground uppercase tracking-widest">
          <span>Band</span>
          <span>Track</span>
          <span>Album</span>
        ...
        {tracks.map((t, i) => (
          <div
            key={i}
            className="grid grid-cols-4 gap-0 border-b border-border px-3 py-2 text-xs hover:bg-muted/50 transition-colors duration-75 group cursor-pointer"
          >
            <span className="text-metal group-hover:text-metal font-medium">{t.band}</span>
            <span className="text-foreground">{t.track}</span>
            <span className="text-muted-foreground truncate">{t.album}</span>
